...
void LevelC::initialise()
{
   ...
   */
   ...
   Entity* jumpBlock = new Entity {
      {mOrigin.x - 11.5f*TILE_DIMENSION, 0},
  ...
  mGameState.collidableEntities[3]->setPosition({mOrigin.x - 9*TILE_DIMENSION , 7*TILE_DIMENSION});
  mGameState.collidableEntities[4]->setPosition({mOrigin.x - 5*TILE_DIMENSION , 5*TILE_DIMENSION});
  mGameState.collidableEntities[5]->setPosition({mOrigin.x - 2*TILE_DIMENSION , 3*TILE_DIMENSION});
  mGameState.collidableEntities[6]->setPosition({mOrigin.x + 2*TILE_DIMENSION , 6*TILE_DIMENSION});
  ...
  for (int i = 0; i < 3; ++i) {
      ...
      flyer->setSpeed(Entity::DEFAULT_SPEED * 0.10f);
      flyer->setColliderDimensions({
         flyer->getScale().x / 2.0f,
         flyer->getScale().y / 2.0f
      });
      flyer->setDirection(DOWN);
      flyer->render(); // calling render once at the beginning to switch ghost's direction
      mGameState.collidableEntities.push_back(flyer);
  }
  ...
  std::cout << "still here " << mGameState.collidableEntities.size() << std::endl;
//   mGameState.collidableEntities[6]->setPosition({mOrigin.x + 2*TILE_DIMENSION , 6*TILE_DIMENSION});
...
void LevelC::update(float deltaTime)
{
   ...
   for (size_t i = 3; i < mGameState.collidableEntities.size(); ++i) {
      if (mGameState.xochitl->getEntityState() == ATTACK) {
         Vector2 charac = mGameState.xochitl->getPosition();
         Vector2 npc = mGameState.collidableEntities[i]->getPosition();
         if (((charac.x -npc.x) < 70 && (charac.x -npc.x) > 0 && mGameState.xochitl->getDirection() == LEFT)
               || ((npc.x - charac.x) < 70 && mGameState.xochitl->getDirection() == RIGHT)
               && abs(npc.y - charac.y) < 30 )
