...
private:
    ...
    Entity* mLastCollidedEntity = nullptr;
    ...
    void checkCollisionY(std::map<std::string, Entity*> collidableEntities);
    void checkCollisionY(Map *map);

    void checkCollisionX(std::map<std::string, Entity*> collidableEntities);
...
public:
    ...
    static constexpr int   DEFAULT_FRAME_SPEED   = 6;  // Lower = slower animation
    ...
    void update(float deltaTime, Entity *player, std::vector<Map*> maps, 
        std::map<std::string, Entity*> collidableEntities);
    void render();
    void normaliseMovement() { Normalise(&mMovement); }

    void jump()       { mIsJumping = true;  }
    void attack()     { mEntityState = ATTACK; mCurrentFrameIndex = 0; mAttackTimer = ATTACK_DURATION; }
    ...
    Entity*     getLastCollidedEntity()    const { return mLastCollidedEntity;    }
